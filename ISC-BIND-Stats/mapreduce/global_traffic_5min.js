...
var finalizef=function (key, value) {
  ...
  var r = {
    "qps": hash_divide(value.counters, 1),
...
var last_processed_cur = db.mr_global_traffic_5min_log.find({}, {
...
if (last_processed_cur.hasNext()) {
  ...
  mr_output=db.runCommand( { "mapreduce":"traffic",
  			      ...
  			      "out": { reduce: "global_traffic_5min" },
  			      "finalize":finalizef
  			    });
  ...
  mr_output=db.runCommand( { "mapreduce":"traffic",
 			      "map": mapf,
 			      "reduce": reducef,
 			      "out": "global_traffic_5min",
 			      "finalize":finalizef
 			    });


  // Create index
  db.global_traffic_5min.ensureIndex({
    "_id.sample_time": 1,
    "_id.pubservhost": 1,
    "_id.zone": 1
  });
...
if (mr_output.ok) {
  print("OK\n");
  var last_processed_cur=db.global_traffic_5min.find({}).sort({"value.created_time":-1}).limit(1);
  if(last_processed_cur.hasNext()){
    var lp = last_processed_cur.next();
    print("Last created_time in global_traffic_5min: " + lp.value.created_time + "\n");  
    db.mr_global_traffic_5min_log.insert({
