...
var last_processed_cur = db.mr_server_stats_hourly_log.find({}, {
...
if (last_processed_cur.hasNext()) {
  ...
  mr_output=db.runCommand( { "mapreduce":"traffic",
  			      ...
  			      "out": { reduce: "server_stats_hourly" },
  ...
  // Create index
  db.server_stats_hourly.ensureIndex({
    "_id.sample_time": 1,
    "_id.pubservhost": 1,
    "_id.zone": 1
  });
...
if (mr_output.ok) {
  print("OK\n");
  var last_processed_cur=db.server_stats_hourly.find({}).sort({"value.created_time":-1}).limit(1);
  if(last_processed_cur.hasNext()){
    var lp = last_processed_cur.next();
    print("Last created_time in server_stats_hourly: " + lp.value.created_time + "\n");  
    db.mr_server_stats_hourly_log.insert({
