...
var last_processed_cur = db.mr_server_stats_daily_log.find({}, {
...
if (last_processed_cur.hasNext()) {
  ...
  mr_output=db.runCommand( { "mapreduce":"server_stats",
 			      "map": map_rescode_daily,
 			      "reduce": reduce_daily,
 			      "out": "server_stats_daily",
 			      "finalize":finalize_daily
 			    });


  // Create index
  db.server_stats_daily.ensureIndex({
    "_id.sample_time": 1,
    "_id.pubservhost": 1,
    "_id.zone": 1
  });
...
if (mr_output.ok) {
  print("OK\n");
  var last_processed_cur=db.server_stats_daily.find({}).sort({"value.created_time":-1}).limit(1);
  if(last_processed_cur.hasNext()){
    var lp = last_processed_cur.next();
    print("Last created_time in server_stats_daily: " + lp.value.created_time + "\n");  
    db.mr_server_stats_daily_log.insert({
