...
var last_processed_cur = db.mr_global_server_stats_hourly_log.find({}, {
...
if (last_processed_cur.hasNext()) {
  ...
  mr_output=db.runCommand( { "mapreduce":"server_stats",
  			      ...
  			      "out": { reduce: "global_server_stats_hourly" },
  ...
  mr_output=db.runCommand( { "mapreduce":"server_stats",
 			      "map": map_rescode_hourly,
 			      "reduce": reduce_hourly,
 			      "out": "global_server_stats_hourly",
 			      "finalize":finalize_hourly
 			    });


  // Create index
  db.global_server_stats_hourly.ensureIndex({
    "_id.sample_time": 1,
    "_id.pubservhost": 1,
    "_id.zone": 1
  });
...
if (mr_output.ok) {
  print("OK\n");
  var last_processed_cur=db.global_server_stats_hourly.find({}).sort({"value.created_time":-1}).limit(1);
  if(last_processed_cur.hasNext()){
    var lp = last_processed_cur.next();
    print("Last created_time in global_server_stats_hourly: " + lp.value.created_time + "\n");  
    db.mr_global_server_stats_hourly_log.insert({
